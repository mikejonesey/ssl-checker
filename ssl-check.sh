...
function checkClient(){
	...
	goodcipher_list="$3"
	badcipher_list="$4"
	client_test=$(openssl s_client -connect $domainname:443 -tls1 -cipher "$goodcipher_list:$badcipher_list" </dev/null 2>&1)
...
function checkPfsCipher(){
	...
	pfs_test=$(openssl s_client -connect $domainname:443 -cipher "$pfs_ciphers" </dev/null 2>&1)
...
function checkDomain(){
	domainname="$1"
	connection=$(openssl s_client -showcerts -connect $domainname:443 </dev/null 2>/dev/null)
