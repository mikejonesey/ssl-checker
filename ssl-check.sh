...
function checkClient(){
	...
	if [ -z "$cipherUsed" ]; then
		...
		if [[ ":$pfs_ciphers:" != *:$cipherUsed:* ]]; then
			echo -e "\e[0;32m  $clientname:  YES : cp: $conProtoUsed sp: $sesProtoUsed c: $cipherUsed (NO FS)\e[0;37m"
		else
			echo -e "\e[0;32m  $clientname:  YES : cp: $conProtoUsed sp: $sesProtoUsed c: $cipherUsed : FS\e[0;37m"
		fi
	fi
}
